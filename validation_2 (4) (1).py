...
def validate2(src,trgt, trgtId, mergedFile, exportfileName):
    ...
    trgt2 = trgt2.fillna("")
    print("lowercase src\n", src2)
    src2_lower = src2.applymap(lambda s: s.lower() if type(s) == str else s)
    trgt2_lower = trgt2.applymap(lambda s: s.lower() if type(s) == str else s)
    print("lowercase src\n", src2)

    columns = src2.columns = trgt2.columns
    src2_lower.columns = trgt2_lower.columns
    resultSet = src2_lower == trgt2_lower
    ...
    for i in columns:
        if (resultSet2[i] == False).sum() > 0:
            ...
            srcMismatchValue = src2.at[mismatchRowNum, i] if src2.at[mismatchRowNum, i] != "" else "Blank"
            trgtMismatchValue = trgt2.at[mismatchRowNum, i] if trgt2.at[mismatchRowNum, i] != "" else "Blank"
            count = (resultSet2[i] == False).sum()
            mismatchCount[i] = f"Mismatchs: {count}"+f"\nEg at row {mismatchRowNum+2}:" + \
                f"\nSrc= {srcMismatchValue}"+f"\nTrgt= {trgtMismatchValue}"
            print(srcMismatchValue, trgtMismatchValue)
...
# main Functions
def execute(srcFile, trgtFile, srcSheet, trgtSheet, srcId, trgtId, exportfilename):
    start = time.time()
    ...
    mismatchCount, validationOutput, srcCount, trgtCount = validate2(src, trgt, importedData, exportfilename)
    ...
    print('total (rows, cols) in merge:', exportfilename.shape)
    # print('total rows in merge:', len(importedData.index))
    
    # calculating process time
    end = time.time()   
    total_time = (end-start)
    print("Process completed in", round(total_time),"seconds or", round(total_time/60, 2) ,"Minutes")
...
layout =[
    [sg.Text('Choose Source file: ', size=(15,1)), sg.InputText(key="srcFile" ), sg.FileBrowse()],
    [sg.Text('Choose Target file: ', size=(15,1)), sg.InputText(key="trgtFile"), sg.FileBrowse()],
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED or event == 'Exit':
            ...
            if srcFile and trgtFile and srcSheet and trgtSheet and srcId and trgtId and exportfilename:
                execute(srcFile, trgtFile, srcSheet, trgtSheet,srcId,trgtId,exportfilename)
            else:
                sg.popup('fill values')
            print(event, values)
